...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.removeIndent = function (root) {
        if (root.body.length <= 0) {
            return;
        }
        var gotIndentString = false;
        var indentString = "";
        var foundFirstStringWithNewLine = false;
        ...
        for (var i = 0; i < root.body.length; i++) {
            ...
            if (!foundFirstStringWithNewLine && !gotIndentString) {
                var newLineIndex = child.indexOf('\n');
                if (newLineIndex >= 0) {
                    foundFirstStringWithNewLine = true;
                    var stringAfterFirstNewLine = child.slice(newLineIndex + 1);
                    if (stringAfterFirstNewLine.length > 0) {
                        var newLineOrCharacter = /[\S\r\n]/;
                        var matchResult = newLineOrCharacter.exec(stringAfterFirstNewLine);
                        if (matchResult) {
                        ...
                        if (indentString.length > 0) {
                            console.log("\"".concat(indentString
                                .replace(/\r/g, '\\r')
                                .replace(/\n/g, '\\n')
                                .replace(/\t/g, '\\t')
                                .replace(/ /g, '|'), "\""));
